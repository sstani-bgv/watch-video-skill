...
- Подключение vidIQ через адаптер текущего инструмента: Codex App или Claude.ai connector для Claude Code.
...
Установочный мастер не привязан к одному агенту. В Codex он открывает штатное подтверждение vidIQ App; в Claude Code использует доступный через аккаунт Claude.ai коннектор `claude.ai Vidiq`. Успех подтверждается наличием реальных `vidiq_*` tools, а не самим показом окна установки. Watch не получает и не хранит пароль vidIQ.
